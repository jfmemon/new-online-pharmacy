...
const PersonalCareItems = () => {
    const [personalCare, refetch] = usePersonalCare();
        ...
        fetch(imageHostingURL, {
            ...
            .then(imageRes => {
                if (imageRes.success) {
                    ...
                    console.log(data);
                    axiosSecure.post('/personalCare', data)
                        .then(data => {
    ...
    const handleDeleteItem = item => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                fetch(`https://online-pharmacy-server.vercel.app/personalCare/${item._id}`, {
                    method: 'DELETE'
                })
                    .then(res => res.json())
                    .then(data => {
    ...
    return (
        <div className='px-10'>
            <div>
                <h3 className='py-4 pl-2 text-2xl font-semibold'>Total items: <span className='text-rose-500'>{personalCare.length}</span></h3>
            </div>
