...
const MyMessages = () => {
  ...
  const handleDelete = item => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        fetch(`https://online-pharmacy-server.vercel.app/contacts/${item._id}`, {
          method: 'DELETE'
        })
          .then(res => res.json())
          .then(data => {
  ...
  return (
    ...
    {
      messages.length == 0 ? <div className='w-full flex flex-col gap-3 justify-center items-center p-5'>
        ...
        <div className='grid min-h-[140px] w-full place-items-center overflow-x-scroll lg:overflow-hidden rounded-lg p-6'>
          <div className='relative flex flex-col bg-clip-border rounded-xl bg-white text-gray-700 shadow-md h-full w-full lg:overflow-hidden overflow-scroll'>
            <table className='w-full min-w-max table-auto text-left'>
              ...
              <tbody>
                {
                  messages.map((item, idx) => <tr key={item._id} className='hover:bg-slate-200'>
